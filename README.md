...
1. **Highlight** an encoded string on a webpage (e.g., `https%3A%2F%2Fexample.com`, `aHR0cHM6Ly9leGFtcGxlLmNvbQ==`, `48656C6C6F`, `\u00A9`, or `&lt;div&gt;`).
...
3. Select one of the following options:
   ...
   - **"Decode as HTML Entity"**: Decodes HTML entities (e.g., `&lt;div&gt;` → `<div>`, `&#65;` → `A`, `&#x41;` → `A`).
...
- **Context Menu**: The `contextMenus` API adds "Decode as URL-encoded", "Decode as Base64-encoded", "Decode as Hex", "Decode as Unicode Escape", and "Decode as HTML Entity" options to the right-click menu when text is selected.
- **Decoding**: The `scripting` API runs a function in the active tab to decode the selected text using:
  ...
  - HTML entity decoding that handles both named entities (e.g., `&amp;`, `&lt;`) and numeric entities (e.g., `&#65;`, `&#x41;`) using DOM APIs.
  - Automatic parsing of common escaped characters like `\n`, `\t`, `\r` regardless of the decoding type used.
- **Output**: The decoded result (or an error message) is displayed in a toast notification that appears in the top-right corner of the browser window. The notification includes a close button and remains visible for 30 seconds with selectable text for user convenience.
...
- **Basic Decoding**: Uses `decodeURIComponent`, `atob`, hex decoding, Unicode escape decoding, and HTML entity decoding, which may not handle non-standard or multi-layered encodings.
- **Toast Notification**: Results appear in a toast notification with selectable text and a close button for easy viewing.
...
- HTML entity decoding that handles both named entities (`&amp;` → `&`) and numeric entities (`&#65;` → `A`, `&#x41;` → `A`)
...
*Last updated: Monday, May 19, 2025 - Added HTML entity decoding and improved toast notifications*